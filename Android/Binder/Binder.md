...
# AIDL
全称是Android Interface Definition Language，它是Android SDK提供的一种机制。借助这个机制，应用可以提供跨进程的服务供其他应用使用。
...
答：核心是**三个类 + 一条链路**。AIDL 生成的 java 文件中只有三个角色（结构详见上文 `# AIDL` 章节）：
...
- **Parcel 序列化受 1M 限制**：参数和返回值都要经过 Parcel 序列化，受 Binder 事务大小限制（约 1M），大对象传输会失败，这也是"为什么 intent 不能传递大数据"的根本原因（见问题 5）。
