...
- NIO与IO，详见 [NIO与IO](./NIO与IO.md)


    - IO 面向流、阻塞；NIO 面向缓冲、非阻塞，selector 可用单线程控制多通道


    - 同步/异步关注的是消息通知的机制，阻塞/非阻塞关注的是程序（线程）等待消息通知时的状态；两两组合出同步阻塞、同步非阻塞、异步阻塞、异步非阻塞


    - BIO 里用户最关心“我要读”，NIO 里最关心“我可以读了”，AIO 里最关心“读完了”；BIO/NIO/多路复用本质都是同步 IO，只有 AIO 是真正的异步 IO
